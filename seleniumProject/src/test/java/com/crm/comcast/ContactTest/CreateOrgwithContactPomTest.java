...
public class CreateOrgwithContactPomTest 
{
	static
	{
...
@Test
	public  void createcontact() throws IOException, InterruptedException
	{
	 ...
	 String Name = r.getExcelData("org",3,0)+"_"+randomnum;
		...
		Contacts contactpg = new Contacts(driver);
		contactpg.getCreatecontact().click();
		
		CreatingContacts crtcontact = new CreatingContacts(driver);
		crtcontact.contactName(ConName);
		
		crtcontact.getOrglookup().click();
		...
		crtcontact.getSavebutton().click();
		
		String title = crtcontact.getTitle().getText();
		if(title.contains(ConName))
		{
		...
		/*WebElement lo = hpg.getLogout();
		wLib.mouseOver(driver, lo);
		hpg.getSignout().click();*/
